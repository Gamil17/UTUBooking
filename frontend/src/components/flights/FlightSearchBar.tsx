  ...
  return (
    <div className="py-3">
      ...
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        {/* Trip type pills */}
        <div className="flex items-center bg-utu-bg-muted rounded-full p-0.5 gap-0.5">
          {TRIP_TYPES.map(({ key, label }) => (
            <button
              ...
              className={`relative px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                params.tripType === key
                  ? 'bg-utu-bg-card text-utu-blue shadow-sm'
                  : 'text-utu-text-muted hover:text-utu-text-secondary'
              } ${key === 'multicity' ? 'cursor-default' : 'cursor-pointer'}`}
            >
              {label}
              {key === 'multicity' && (
                <span className="ms-1 text-[9px] text-utu-text-muted font-normal">{t('comingSoon')}</span>
              )}
            </button>
          ))}
        </div>
        ...
        <select
          ...
          className="text-xs border border-utu-border-default rounded-full px-3 py-1.5 bg-utu-bg-card text-utu-text-secondary focus:outline-none focus:ring-2 focus:ring-utu-blue"
        >
        ...
        {/* Adults counter */}
        <div className="flex items-center gap-2 border border-utu-border-default rounded-full px-3 py-1 bg-utu-bg-card">
          <button
            type="button"
            onClick={() => set('adults', Math.max(1, params.adults - 1))}
            className="w-5 h-5 flex items-center justify-center text-utu-text-muted hover:text-utu-blue"
          ...
          <span className="text-xs font-semibold text-utu-text-secondary w-10 text-center">
      ...
      <div className="flex items-end gap-2 flex-wrap lg:flex-nowrap">
        ...
        <div className="flex-1 min-w-[120px]">
          <label className="text-[10px] font-semibold text-utu-text-muted uppercase tracking-wide block mb-1">{t('from')}</label>
          <input
            type="text"
            value={params.from}
            ...
            maxLength={3}
            className="w-full border border-utu-border-default rounded-xl px-3 py-2.5 text-sm font-semibold text-utu-text-primary placeholder:text-utu-text-muted placeholder:font-normal focus:outline-none focus:ring-2 focus:ring-utu-blue uppercase"
          />
        </div>
        ...
        <button
          ...
          onClick={swap}
          title={t('swap')}
          className="w-9 h-9 mb-0.5 flex items-center justify-center rounded-full border border-utu-border-default bg-utu-bg-card hover:bg-utu-bg-subtle hover:border-utu-border-default text-utu-text-muted hover:text-utu-blue transition-colors shrink-0"
        >
          <SwapIcon />
        </button>
        ...
        {/* To */}
        <div className="flex-1 min-w-[120px]">
          <label className="text-[10px] font-semibold text-utu-text-muted uppercase tracking-wide block mb-1">{t('to')}</label>
          <input
            type="text"
            value={params.to}
