...
function SeatsIcon() {
  return (
    <svg className="w-3.5 h-3.5 text-gray-400 shrink-0" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
function TransIcon() {
  return (
    <svg className="w-3.5 h-3.5 text-gray-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8} aria-hidden="true">
...
function CarPlaceholder() {
  return (
    <svg className="w-16 h-16 text-gray-300" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
...
const SUPPLIER_COLORS: Record<string, string> = {
  ...
  hertz:      'bg-yellow-400 text-gray-900',
...
function supplierChip(name: string) {
  ...
  return SUPPLIER_COLORS[key] ?? 'bg-gray-700 text-white';
}
...
export default function CarCard({ offer, isTopPick, onSelect }: Props) {
  ...
  const image = (offer as any).images?.[0] ?? null;
  const days  = (offer as any).days ?? (offer.totalPrice && offer.pricePerDay
    ? Math.round(offer.totalPrice / offer.pricePerDay) : null);

  const fuelType      = (offer as any).fuelType      ?? null;
  const mileage       = (offer as any).mileageIncluded ?? null;
  const bags          = (offer as any).bags           ?? null;
  const doors         = (offer as any).doors          ?? null;
  const fuelPolicy    = (offer as any).fuelPolicy     ?? null;
  const pickupType    = (offer as any).pickupType     ?? null;
  const freeCancellation = (offer as any).freeCancellation ?? false;
  const rating        = (offer as any).rating         ?? null;
  const reviewCount   = (offer as any).reviewCount    ?? null;
  const reviewLabel   = (offer as any).reviewLabel    ?? null;

  return (
    <article className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-3 overflow-hidden hover:shadow-md transition-shadow">
      <div className="flex flex-col sm:flex-row">

        {/* ── Image ────────────────────────────────────────────────── */}
        <div className="relative sm:w-52 sm:shrink-0 h-40 sm:h-auto bg-gray-50 flex items-center justify-center overflow-hidden">
          {isTopPick && (
          ...
          {image ? (
            <img src={image} alt={offer.name} className="w-full h-full object-contain p-2" loading="lazy" />
          ) : (
            <CarPlaceholder />
          )}
        </div>
        ...
        <div className="flex-1 p-4 flex flex-col gap-2 min-w-0">
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <h3 className="font-bold text-gray-900 text-sm leading-tight">
                {offer.name}
                <span className="ms-1.5 text-xs font-normal text-gray-400">{t('orSimilar')}</span>
              </h3>
              <span className="text-[10px] text-gray-500 capitalize">{offer.category}</span>
            </div>
            ...
            <div className="shrink-0 text-end">
              ...
              <div className="text-lg font-extrabold text-gray-900 leading-none">
                SAR {offer.totalPrice.toLocaleString('en-SA')}
              </div>
              <div className="text-[10px] text-gray-400">{t('totalPrice')}</div>
              <div className="text-xs text-gray-500">
                SAR {offer.pricePerDay.toLocaleString('en-SA')} {t('perDay')}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-1.5 text-[10px]">
            ...
            {offer.pickupLocation && (
              <span className="text-gray-400 truncate">{offer.pickupLocation}</span>
            )}
          </div>
          ...
          <div className="mt-auto pt-1 flex items-center gap-3 flex-wrap">
            ...
            {rating != null && (
              <div className="flex items-center gap-1">
                ...
                {reviewLabel && <span className="text-xs text-gray-500">{reviewLabel}</span>}
                {reviewCount != null && (
