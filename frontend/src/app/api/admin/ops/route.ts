 ...
 *
 * ?view=stats
 * ?view=incidents&status=&severity=&service=&page=&limit=
 * ?view=tickets&status=&priority=&category=&search=&page=&limit=
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    const up = await fetch(`${ADMIN}/api/admin/ops/${endpoint}${suffix}`, {
      headers: hdr,
      cache:   'no-store',
