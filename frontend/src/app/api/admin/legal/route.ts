 ...
 *
 * ?view=stats
 * ?view=matters&status=&type=&urgency=&jurisdiction=&page=&limit=
 * ?view=tasks&status=&type=&jurisdiction=&page=&limit=
 * ?view=documents&type=&status=&jurisdiction=&page=&limit=
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    const up = await fetch(`${ADMIN}/api/admin/legal/${endpoint}${suffix}`, {
      headers: hdr,
      cache:   'no-store',
