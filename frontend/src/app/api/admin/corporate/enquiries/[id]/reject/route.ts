...
/** POST /api/admin/corporate/enquiries/:id/reject — reject application + email applicant */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const up = await fetch(
      `${ADMIN}/api/admin/corporate/enquiries/${params.id}/reject`,
      { method: 'POST', headers: hdr, body: JSON.stringify(body), signal: AbortSignal.timeout(10_000) }
    );
