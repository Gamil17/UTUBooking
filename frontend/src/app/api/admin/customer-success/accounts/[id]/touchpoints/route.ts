...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const up = await fetch(`${ADMIN}/api/admin/customer-success/accounts/${id}/touchpoints${qs ? '?' + qs : ''}`, {
      headers: hdr,
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const up   = await fetch(`${ADMIN}/api/admin/customer-success/accounts/${id}/touchpoints`, {
