...
const TIERS = [
  ...
  {
    key:       'Elite',
    color:     'bg-emerald-50 border-emerald-400 ring-2 ring-emerald-300',
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-emerald-900 py-20 px-4 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          ...
          <Link
            href="/login"
            className="inline-block border border-emerald-400 hover:bg-emerald-800 text-emerald-100 font-medium px-8 py-3 rounded-xl transition-colors text-sm"
          >
            {t('signIn')}
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {[
            ...
            <div key={step.num} className="bg-utu-bg-card rounded-2xl border border-utu-border-default p-6 shadow-sm text-center">
              ...
              <p className="text-xs font-bold text-emerald-600 mb-1">{step.num}</p>
      ...
      <section className="bg-utu-bg-card border-y border-utu-border-default py-12 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {EARN_RATES.map((item) => (
              <div key={item.labelKey} className="flex flex-col items-center text-center p-4 rounded-xl bg-slate-50 border border-utu-border-default">
                <span className="text-2xl mb-2">{item.icon}</span>
                <p className="text-sm font-semibold text-utu-text-primary mb-1">{t(item.labelKey)}</p>
                <p className="text-xs text-emerald-600 font-medium">{t(item.rateKey)}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-emerald-900 py-16 px-4 text-center">
        ...
        <p className="text-emerald-100 mb-6 max-w-md mx-auto text-sm">{t('ctaDesc')}</p>
        <Link
          href="/register"
          className="inline-block bg-amber-400 hover:bg-amber-300 text-emerald-900 font-bold px-8 py-3 rounded-xl transition-colors text-sm"
        >
